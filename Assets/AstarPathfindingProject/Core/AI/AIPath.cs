...
namespace Pathfinding {
	...
	public partial class AIPath : AIBase, IAstarAI {
		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			//	if (reachedEndOfPath && aIDestinationSetter.allWP.Count == 0)
			//	{
			//		OnTargetReached();
			//		print("Finished");
			//		aIDestinationSetter.allWaypointsSearched = true;
			//	}
			//	else if (reachedEndOfPath && aIDestinationSetter.allWP.Count != 0)
			//	{
			//		GetNewPaths();
			//	}
   //         if (aIDestinationSetter.allWaypointsSearched)
   //         {
			//	if(aIDestinationSetter.finalDestination != null && aIDestinationSetter.finalDestinationReached == false)
   //             {
			//		aIDestinationSetter.target = aIDestinationSetter.finalDestination;
			//		if(aIDestinationSetter.target == aIDestinationSetter.finalDestination && !prevTargetReached && reachedEndOfPath)
   //                 {
			//				aIDestinationSetter.finalDestinationReached = true;
			//				OnTargetReached();
   //                 }							
   //             }
   //         }
			//if (!prevTargetReached && reachedDestination)
			//{
			//	aIDestinationSetter.RemoveLastOnList();
			//}
			//End Modification
		...
		{
			AIDestinationSetter destinationSetter = GetComponentInChildren<AIDestinationSetter>();
			...
			//gets a random wp
			//int waypointIndex = Random.Range(0, destinationSetter.allWP.Count);
			//sets the target to the random wp

			Transform nextWp = null;

			if (!isChasing)
			{
				destinationSetter.allWP.RemoveAt(0);
				nextWp = destinationSetter.allWP[0];
				destinationSetter.target = nextWp;

				//once reached target remove it from the list and add it to saved wp
				if (reachedDestination)
				{
					destinationSetter.savedWP.Add(nextWp);
					destinationSetter.lastwpTarget = nextWp;
				}
			}
		}
		// End Modifications
